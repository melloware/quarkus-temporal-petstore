...
- [Quarkus Mailpit Extension](https://github.com/quarkiverse/quarkus-mailpit) for zero-config SMTP capture and an email UI
- [PostgresSQL](https://www.postgresql.org/) the world's most advanced open source relational database
- [Grafana LGTM](https://github.com/grafana/docker-otel-lgtm) for distributed tracing, metrics, and logs using OpenTelemetry
...
You need Docker or Podman running, but you can choose how the supporting infrastructure is
started. **Dev Services is the default and needs no setup at all**; Docker Compose remains
available if you would rather manage the stack yourself.

### Option A: Dev Services (default)

Just run the services. Quarkus starts every container they need, shares one container per
piece of infrastructure across all 6 services, and shuts everything down when you stop:

| Infrastructure | Provided by                                       |
| -------------- | ------------------------------------------------- |
| Temporal       | `quarkus-temporal` Dev Service (server + Web UI)  |
| PostgreSQL     | Quarkus Dev Services (order-service)              |
| Mailpit        | `quarkus-mailpit` Dev Service (notification-service) |
| Grafana LGTM   | Quarkus Observability Dev Services                |

Because these containers get ephemeral ports, find their UIs on the Dev UI cards at
`http://localhost:<service-port>/q/dev-ui/`. The Mailpit UI is proxied at a fixed path,
[http://localhost:8089/q/mailpit](http://localhost:8089/q/mailpit).

> [!TIP]
> Add `testcontainers.reuse.enable=true` to `~/.testcontainers.properties` to keep the
> containers alive between restarts instead of recreating them each time.

### Option B: Docker Compose

Start the stack yourself, then run the services with the `compose` profile:
...
| Grafana    | OpenTelemetry traces, metrics and logs UI (admin/admin)    | [http://localhost:3000](http://localhost:3000)   |
| PostgreSQL | Database for both the orders and for Temporal storage      | 5432                                             |

Unlike Dev Services, everything here is on a fixed, predictable port.

## Running Workflow

First, from the root of the project you must compile and build all dependencies using `mvn clean install`.

Next, you will need to have all 6 services running, each in its own command prompt window for
easy testing and startup/shutdown:

```shell
$ mvn quarkus:dev                              # Option A: Dev Services
$ mvn quarkus:dev -Dquarkus.profile=compose    # Option B: Docker Compose
```
